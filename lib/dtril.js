...
*/
function dtril( order, M, N, k, A, LDA, B, LDB ) {
	var isrm;
	var sa1;
	var sa2;
	var sb1;
	...
	if ( l === null ) {
		throw new TypeError( format( '36VFx', order ) );
	}
	isrm = isRowMajor( l );
	if ( isrm ) {
		s = N;
	...
	if ( LDA < max( 1, s ) ) {
		throw new RangeError( format( '36VJL', s, LDA ) );
	}
	if ( LDB < max( 1, s ) ) {
		throw new RangeError( format( '36VGM', s, LDB ) );
	}
	if ( isrm ) {
		sa1 = LDA;
		sa2 = 1;
